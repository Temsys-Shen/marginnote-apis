...
<Accordions>
  <Accordion title="confirm 门清单（不带即 422 且不执行）">
    ...
    **无此门**（发即执行）：`prepare` / `submit` / `focus` / `discard`、打快照（`study-sets/{id}/snapshots`）、`ui-state/apply`、`install/code`。别拿它们试参数；`submit` 校验用 `GET /bundles`。完整纪律见[危险端点纪律](/docs/reference/dangerous-endpoints)。
  </Accordion>
